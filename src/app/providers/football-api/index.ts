...
export async function getCachedTransferData(): Promise<Doc<"transfers">[]> {
	// Bump this when forcing refresh to avoid any higher-level memoization.
	void transfersCacheBust;
	try {
...
export async function refreshTransferData(): Promise<{
	...
	try {
		...
		// Native cache busting for environments without Next.js cache tags
		transfersCacheBust++;
